-- Convene Pass 1: publish_post's convene branch writes the event object (P1-SPEC section 4).
-- One write path stays (the absolute): the composer's Convene form stores its keys namespaced
-- `convene.{key}` (ruling 664, 681) and this function is the only thing that turns them into rows.
-- Latest prior definition: 20260912090557_fix_pr_02_rulings_408_444.sql line 907. Every other
-- branch is byte for byte what that file carries.
--
-- The namespaced keys this branch reads from payload -> 'fields' (the client's contract, written by
-- src/components/dna/ConveneForm.tsx through src/lib/publish.ts):
--   convene.title                 the title; the body's first line stands in when absent
--   convene.format                in_person | online | hybrid (SPEC 3, the door); online is stored as
--                                 event_mode 'virtual', which the card renders as the word Online
--   convene.when                  the member's words for the moment (when_text)
--   convene.when_window           the member's words for an honest window (520), e.g. 'November'
--   convene.starts_at, convene.ends_at, convene.doors_at
--                                 ISO instants the client parsed in the event's time zone (PR 3);
--                                 the function validates the 520 and 634 invariants and never parses
--   convene.expected_window_start, convene.expected_window_end
--                                 ISO dates when the window's words resolved to a month (520)
--   convene.timezone              IANA zone derived from the place, or the host's browser zone for an
--                                 online event (Pass 1 handoff, ruling owed 7); never asked
--   convene.place_id, convene.place_name, convene.city, convene.country, convene.lng, convene.lat
--                                 the resolved place (place-resolve); convene.place_text the words
--                                 when zero or several results stood (publishable, SPEC 3)
--   convene.link                  the meeting link; convene.link_tba true for to-be-announced (521)
--   convene.delivery_intent       the sentence the card and page render (676)
--   convene.price_nature          free | paid | donation (ticket_kind)
--   convene.capacity              host-only integer, stored in event_host_settings (623)
--   convene.space_id              Canon 6; the events insert policy checks membership
--   convene.meta, convene.presented_by, convene.when_date, convene.when_time, convene.ends,
--   convene.doors, convene.place_query
--                                 preview, read-back and picker state the form keeps; not stored
--                                 (674: presented_by is the poster's name, read from members)
...
begin
  ...
  if v_verb = 'connect' then
    ...
    if v_format in ('in_person', 'hybrid') then
      insert into public.event_delivery (event_id, kind, position, place_id, place_name, place_text, city, country, lng, lat)
      values (
        v_obj_id, 'physical', 0,
        v_place_id,
        v_place_name,
