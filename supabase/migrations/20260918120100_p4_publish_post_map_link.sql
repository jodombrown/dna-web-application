-- Convene Pass 4: publish_post carries the host's own map link (P4-SPEC section 7; rulings 815,
-- 816). Committed before it is applied (ruling 225); reaches the canonical project by
-- `supabase db push` from the founder's machine, never by apply_migration (rulings 553, 269).
--
-- A new file rather than an edit to 20260916120200 (ruling 466): an applied migration is never
-- amended, and `tests/migration-lint.cjs` enforces that by version rather than by path.
--
-- Latest prior definition: 20260916120200_p1_publish_post_convene.sql. Every branch of that file is
-- carried here byte for byte; this definition differs from it in exactly four places, all inside
-- the convene branch and all about `convene.map_link`: the declaration, the read, the three
-- refusals, and the column on the physical `event_delivery` row.
--
-- What Pass 4 does NOT add to this function, and why. The host-placed pin has no coordinate to
-- store. The map plate this pass ships is a drawn plate — a hairline grid, no tiles and no invented
-- geography, which is P4-SPEC's own guardrail — and a plate with no tiles carries no projection, so
-- a position on it is a position on a plate and not a place on the earth. Writing it into `lng` and
-- `lat` would be invented geography in the database, which is the worst form of the thing the
-- guardrail refuses. The composer holds the host's point as a fraction of the plate, the payload
-- carries `convene.pin_x` and `convene.pin_y`, and this function reads neither: an unread key is
-- dropped, which is the correct outcome and is what makes the omission provable rather than
-- promised. Logged as gap G43; the pin gains a coordinate in the pass that gives the plate tiles.
--
-- The keys this branch reads are 20260916120200's list plus one:
--   convene.map_link            the host's own link to a map, kept opaque (815, 816). Never parsed,
--                               never geocoded, never plotted, and no point, place or zone is
--                               derived from it here or anywhere in the tree. The only test it
--                               faces is the shape an href must have before a surface renders it.
...
begin
  ...
  if v_verb = 'connect' then
    ...
    if v_format in ('in_person', 'hybrid') then
      insert into public.event_delivery (event_id, kind, position, place_id, place_name, place_text, city, country, lng, lat, map_link)
      values (
        v_obj_id, 'physical', 0,
        v_place_id,
        v_place_name,
