-- Convene Pass 2 (Brief 10, Attend): every event gets its public link. Ruling 1024, on 662, 679 and
-- 1028. Committed before it is applied (ruling 225). Applied to the canonical project by Chat through
-- the Supabase MCP's execute_sql under rulings 963 and 965, with its supabase_migrations.schema_migrations
-- row in the same transaction, and never by apply_migration (rulings 553, 269). Runs after
-- 20260921140100.
--
-- The link (1024). public.events.slug is the public page's address, /e/{slug}: the title's words, at
-- most sixty characters of a to z and 0 to 9 joined by hyphens ("event" when the title has none),
-- then a hyphen and six random hexadecimal characters. private.new_event_slug makes one and is a
-- definer so its collision check reads every event, not only the ones the publishing member can see.
-- The publish path writes it once, in publish_post's events insert, and the existing events are
-- backfilled here. It never changes after that, so a shared link keeps working through any edit to
-- the title: events_slug_unchanged refuses an update that would change it. That trigger is a guard on
-- the row itself, not a derived row, so 1002 does not reach it. Whether an event has a public page at
-- all is 1028's question and is answered by the page's projection in 20260921160100, not by the slug.
--
-- publish_post is replaced with its body exactly as 20260919120100 last defined it, which is the live
-- body (md5 280c76a4aa2b4a9d6fc07c0e850294d4 before this file), with the one change 1024 makes: its
-- events insert writes slug. Its grants carry over, because create or replace keeps them.

create function private.new_event_slug(p_title text)
returns text
language plpgsql
volatile
security definer
...
revoke execute on function private.new_event_slug(text) from public, anon;
grant execute on function private.new_event_slug(text) to authenticated, service_role;

alter table public.events add column slug text;

update public.events e set slug = private.new_event_slug(e.title) where e.slug is null;
...
alter table public.events add constraint events_slug_check check (slug ~ '^[a-z0-9]+(-[a-z0-9]+)*$');

-- The link never changes once written (1024).
create function private.events_slug_unchanged()
returns trigger
language plpgsql
set search_path = ''
as $$
begin
  if new.slug is distinct from old.slug then
    raise exception 'An event''s link never changes once it is published.' using errcode = '22023';
  end if;
  return new;
end;
$$;

revoke execute on function private.events_slug_unchanged() from public, anon, authenticated;

create trigger events_slug_unchanged
  before update of slug on public.events
  for each row execute function private.events_slug_unchanged();

-- publish_post, as 20260919120100 last defined it, with its events insert writing slug (1024).
...
begin
  ...
  if v_verb = 'connect' then
    ...
    insert into public.events (
      ...
      window_basis, delivery_intent, slug
    ) values (
      ...
      v_window, coalesce(f ->> 'convene.delivery_intent', ''), private.new_event_slug(v_title)
