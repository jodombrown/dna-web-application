// Company email addresses (ruling 387, Notion D489; supersedes ruling 385 / D487).
//
// The DNA Email Directory in Notion is the source of truth for every address, its purpose and where
// it is published. This module is the one place the code reads them from: no surface and no edge
// function carries an address literal, and a repository scan (tests/contact.cjs) fails the build if
// one appears anywhere but here and the Deno mirror at supabase/functions/_shared/contact.ts, which
// must stay value-for-value identical (the parity arm of the same test proves it).
//
// These are build-time constants shared by the client bundle and the Deno edge runtime. They are
// deliberately not a runtime vocabulary table: a network read to render an address would be a
// regression, and the edge runtime cannot depend on a client fetch.
//
