...
- Migrations: `20260908200000_b4_connect_tables.sql` (`edges` with the `edge_type` enum, `member_connections`, `connection_requests.message` and `responded_at` with the one-pending-per-pair index, `second_degree`, `dismissed_suggestions`, `corridors` and `member_corridors` at zero rows, `member_embeddings` with its HNSW index and no writer, `member_blocks`, `member_segments`, `private.connect_settings` with `where_floor` 5 and `decline_window_days` 90), `20260908200100_b4_connect_rls.sql` (default-deny policies for every persona; the sender has no select on `connection_requests` and no member has update or delete; a follow is the follower's alone; `via_count` is withheld by column grant; accept writes both connect edges and both adjacency rows by trigger, adjacency maintains `second_degree` incrementally, `private.refresh_second_degree()` rebuilds it nightly through pg_cron; a follow edge mirrors into `member_follows` for the profile's read), `20260908200200_b4_connect_rpcs.sql` (`private.relationship_state`, `private.connect_card`, `connect_cards(lens, filters, cursor, limit)`, `connect_where()`, `connect_filter_options()`, `send_introduction`, `respond_to_request`, `withdraw_request`, `set_follow`, `dismiss_suggestion`, the `connection_request_intros(ids)` projection, and `profile_view` and `publish_post` amended to the new relationship rule and the request message).
