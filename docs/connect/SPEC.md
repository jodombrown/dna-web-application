...
15. **Blocks**: the chassis had no block store, so `member_blocks` ships here (own rows, no surface in this brief). Ruling 186 moves its ownership to the chassis; the table stays. `private.is_blocked` filters Members, Suggested, the network lens, the rail rows, every mutual name and the Where count in both directions, audited live rather than read (`docs/GAPS.md`, audit table). Two projections did not filter: `connection_request_intros` returned the other party's name and is fixed; `profile_view`'s own subject is still readable to a blocked member and is logged as gap G2, because what block does to a profile is a product decision no ruling settles. The table has no writer anywhere in the app: gap G1.
