# Strand `_ds/` export — compile `v1790279130697923`
...
> **New at v1790279130697923.** `MediaBlock` gallery without `ratio`: rows declared as correction 26 declares them with a ratio, so three and four images fill the 16/10 frame instead of clipping the lower row. Two images: a stated style difference, box sizes unchanged. Everything else identical to `v1790276435314618`. `STRAND-CORRECTION-27-EXTRACTION.md` has the record and the 29 readings.

> **Static check for this id:** first line of `_ds_bundle.js` reads `/* @ds-compile-id: v1790279130697923 */`; runtime `window.__DS_COMPILE_ID === 'v1790279130697923'`.
