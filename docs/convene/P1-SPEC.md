# Convene Pass 1, Compose and card: SPEC as a divergence list (495)

**15 September 2026. Base: Brief 1's Universal Post Composer (`composer/B1-Composer-v3.dc.html`, built and merged under 77 and 145) on Strand's Composer and Sheet; Brief 2's Feed card, the chassis PostCard with 105's expanded state (`shell/B2-Shell-Feed-v3.dc.html`, built).** Approved under 62 (677); Revision 3 ratified 683; Revision 4 line under 705; speakers-row Revision 3 line under Brief 10 (ruling owed 3, default). Strand compile `v1789537371639386`. Code builds the base as built and applies only the deltas below; everything not named here is as built.

## Pages

- `convene/P1-Compose-and-Card-v1.dc.html` with `convene/p1-convene-form.js` (the Convene form Convene supplies to Strand's Composer through `forms.convene`) and `composer/dia-mock.js` (prototype-only inference mock)

## Deltas by scope item and ruling

| Scope item | Delta from the base | Ruling |
|---|---|---|
| Step 3 form slot | The Composer takes `forms.convene`, a per-verb form Convene owns; five named sections (the invitation, the moment, the door, the program, the send-off) then More options; fields that do not apply are absent | 580, 621, 664 |
| Format first | In person, Online, Hybrid as Strand's Segment; nothing else in the door renders until chosen | 621 |
| Place resolved, never typed | Venue name lookup narrowed by the member's homes; DIA may fill the venue and never the city; free text held in `place_text` | 633, 581 |
| When in the member's words | Parsed and read back in one quiet line; a failed parse offers the picker; an honest date window as words, no end time invented | 520, 634 |
| Proposal without selection | DIA proposes the verb and selects nothing; Publish stays off until the member taps a chip | 635, 667, 668 |
| Disabled chip with a reason | Inside an event the Convene chip stays in the row disabled; the reason writes into the DiaLine slot on a touch tap | 672, 681 |
| Preview | Reads only `convene.title` and `convene.meta`; carries the card's fixed actions | 671, 673 |
| Publish states | In flight and failed through Sheet's `loading` and `error`; the host builds the card from `created_object.convene.*` | 665, 666, 664 |
| Tickets and format controls | Strand's Segment | 681 |
| The card | Meta `Presented by {presented} · {when} · {where}`; rows only with a record: Going (three names then and others at five or more), Sponsor, Speakers; cancelled and past as full treatments; Ask the host as the Respond label | 508, 630, 642, 643, 674, 678 |
| Event object | Fields per P1-SPEC section 4 including the calendar-ready set; `attachments` is a Hub field, never compose-time | 621, 705 |

Everything else is Brief 1's Composer and Brief 2's card as built.

---

## The full spec, as approved (`convene/P1-SPEC.md` Revision 3)
...
- **Place, resolved never typed.** `Place` takes a venue name. Empty: `Near one of your homes` with the member's homes as chips (633); a home narrows the lookup. Typing: one unambiguous result settles as resolved after a pause; the field becomes a row (`map-pin`, `Front Room, Osu, Accra`, `Change`) with `Time zone GMT, from the place.` Zero results: hint `No place found for that. It is kept as you wrote it.` and the words stand in `place_text`. Several: hint `Several places match. Pick one, or leave it as you wrote it.` over a listbox of the matches; nothing is chosen for the member. Both fallbacks are publishable.
...
**States drawn:** proposal without selection; chip tapped after the read; in person; online; hybrid; place empty with homes; place resolved; place with no result; place with several; when parsed; when failed with picker; date window; More options open; publish in flight; publish failed; published; chip disabled. Each on three tiers, both themes, touch under expanded and pointer at expanded.
...
| `place_id`, `place_name`, `city` | place resolved to one result; DIA may fill `place_name`, never `city`; free text fallback held in `place_text` |
...
| `delivery_intent` | the sentence the card and page render |
