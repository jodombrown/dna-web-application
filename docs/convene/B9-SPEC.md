...
Handoff spec for Claude Code. **Revision 2, 25 Sep 2026 (Session 33), written by Chat against the tree** (`docs/convene/B9-SPEC.md` at `d23c55f`) under rulings 1124 to 1132. It replaces the 23 Sep text and Design's 25 Sep rewrite, which is withdrawn (1132). The register governs this spec; a line here overrides a ruling only where the ruling that amends it is named (1126). The repo copy is authoritative on conflict (762, 763). Strand tokens only (604). No count, badge or number anywhere (rule 6).

The drawn reference is `convene/B9-Discovery-Dashboard-v5.dc.html` in the app Design project. Its frames bind Strand correction 24, `v1790160131064912` (262,888 bytes, sha256 `18cc0e4e6701df615602049d117bac5cd34c43f76795a1c0ee6e0dd64cd0f1e3`, read by the app project on 25 Sep), which predates corrections 25 to 27. Every part is read from the tree's compile, `v1790279130697923`, and never measured off the frames (924). The app project rebinds once, after Chat ratifies correction 29.
...
- Pane open, expanded only (688, 1063 to 1068, 1083, 1127): grid `minmax(0,1fr) 520px`, the pane 520 at the right and the list taking the rest, gap `--pane-gap`, height = frame − header − 88; list column and pane body scroll separately; the header row moves into the list column so rail, list and pane start level; FacetRail collapsed to its strip; the list follows `selectedKey` vertically and within the lane. Toolbar: Hide or show the list, Copy link, Share. Pane's cluster: Previous event, Next event, Back to Discovery; edge items `aria-disabled` in place; ArrowLeft and ArrowRight while focus is in the pane. Hide list → `0 minmax(0,720px)` centred. **The 520 pane, the toolbar and Hide list need Strand correction 29's Pane reading width (1127, 1130); until the re-sync lands, the tree's pane stands as built and G110 stays open.**
...
- Elsewhere: the names going (1124), only when five or more are going, each name within that attendee's own RSVP visibility as `event_page` resolves it (508, 645, 680, 1128): three names then "and others are going". Below five, or with fewer than three names visible to the viewer, the row is empty. Real names only, never a count. The names come from one server read beside `event_presenters`; no name lookup in TypeScript. Needs correction 29's going slot on the face (1130).
...
Owed by correction 29, or 28 if 28 has not compiled (1130, 1133): the discovery face's going slot and `onReport` slot; Pane's fixed reading width (1127); the Browse tile. For the second handoff: MediaBlock's group-of-four strip and five-slide carousel, BodyBlocks, the Person card, the `flag` icon.
